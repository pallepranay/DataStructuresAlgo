...
public:
    ...
    {
        if(i < 0 and j < 0) return true; // if we traversed both, that means pattern covered the whole string
        if(i >= 0 and j < 0) return false; // string is left, but pattern khatam(pattern is over)
        if(i < 0 and j >= 0) { // string khatam, but pattern baaki h, to check kro(string is over but pattern left)
